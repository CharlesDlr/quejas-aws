...
//Auditor
router.get("/quejasauditor/:idpage", authorize, rolauth, async (req, res) => {
  try {
    ...
    if (rol === "SuperAdmin") {
      ...
      const quejas = await pool.query("select q.queja_id Id, s.nombre Sucursal, e.nombre Ejecutivo, tq.nombre TipoQueja, q.descr Descripción, q.fecha Fecha, q.estatus from quejas q inner join ejecutivos e  on e.ejecutivo_id=q.ejecutivo_id inner join sucursales s  on e.sucursal_id=s.sucursal_id inner join origen o on q.origen_id=o.origen_id inner join tipo_queja tq on tq.tipo_queja_id=q.tipo_queja_id where s.sucursal_id=$1 order by Id asc limit 10 offset $2;", [sucursal.rows[0].sucursal_id,(10*(req.params.idpage-1))]);
